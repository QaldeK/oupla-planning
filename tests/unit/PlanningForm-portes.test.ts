...
/** Bouton « Edit slot times » (pencil) : toujours rendu, 1 par slot. */
function slotCount() {
	return screen.getAllByRole("button", { name: /edit slot times/i }).length;
}

/** Boutons « Delete this slot » : rendus ssi `timeSlots.length > 1`. */
function deleteSlotButtons() {
	return screen.queryAllByRole("button", { name: /delete this slot/i });
}
...
describe("PlanningForm — Porte 5 (removeTimeSlot)", () => {
	it("édition sans DateSlots actives (cycle passé) → pas de modal, slot supprimé directement", async () => {
		...
		// Cliquer « Delete this slot » sur le 2e slot (s2)
		await user.click(deleteSlotButtons()[1]);

		// Pas de ConfirmModal
		expect(screen.queryByRole("dialog")).not.toBeInTheDocument();

		// s2 supprimé : on repasse à 1 slot → tous les boutons « Delete » disparaissent
	...
	it("édition avec N DateSlots actives → modal avec message compté, puis slot supprimé", async () => {
		...
		// Cliquer « Delete this slot » sur le 1er slot (s1)
		await user.click(deleteSlotButtons()[0]);

		// La ConfirmModal s'ouvre (titre « Delete this slot »)
		const dialog = await screen.findByRole("dialog");
		expect(dialog).toHaveTextContent(/delete this slot/i);
		// Message avec le compte exact (3 DateSlots actives → « The 3 occurrences… »)
		expect(dialog).toHaveTextContent(/the 3 occurrences for this slot/i);

		// Cliquer « Delete » dans la modal → s1 supprimé
	...
	it("création → suppression directe, pas de modal (comportement inchangé)", async () => {
		...
		// Ajouter un 2e slot via le modal « Add slot » (draft pré-rempli
		// avec les horaires du dernier slot, il suffit de cliquer « Apply »)
		...
		// Cliquer « Delete this slot » sur le 2e slot
		await user.click(deleteSlotButtons()[1]);
