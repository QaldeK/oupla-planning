...
/** Formate une date YYYY-MM-DD comme le fait le badge (EEE d MMM).
 * Locale de test = baseLocale "en" → badges rendus au format en-US. */
...
function getDateBadge(iso: string) {
	return screen.getByRole("button", {
		name: new RegExp(formatBadgeDate(iso).replace(/\./g, "\\."), "i")
	});
}
...
// porte la legend (« From » / « To ») qui sert de nom accessible.
...
describe("PlanningForm — Porte 1 (requestRecurrenceTypeChange)", () => {
	it("édition : changer le type de récurrence ouvre la ConfirmModal", async () => {
		...
		// ConfirmModal porte 1 (titre « Change recurrence type »).
		const dialog = await screen.findByRole("dialog");
		expect(dialog).toHaveTextContent(/change recurrence type/i);
...
describe("PlanningForm — Porte 2 (requestDateChange)", () => {
	it("édition : exclure une date avec données ouvre la ConfirmModal", async () => {
		...
		await setDate(user, getDateInput("From"), "2026-08-16");

		// ConfirmModal porte 2 (titre « Edit period »).
		const dialog = await screen.findByRole("dialog");
	...
	it("édition : changer une borne sans exclure de date avec données ne confirme pas", async () => {
		...
		await setDateOverwrite(user, getDateInput("From"), "2026-08-05");

		expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
		// L'input reflète la nouvelle valeur (changement direct).
		expect(getDateInput("From").value).toBe("2026-08-05");
	});
	...
	it("CUSTOM : les inputs firstDate/lastDate ne sont pas rendus", () => {
		...
		// fieldsets From/To : pas de cycle généré, dates libres via MultiDatePicker.
...
describe("PlanningForm — Porte 3 (requestRemoveManualDate)", () => {
	it("édition : supprimer une date manuelle avec données ouvre la ConfirmModal", async () => {
		...
		// Le popover mono-slot + manualDate affiche « Delete » (et non
		// « Disable »). À ce stade, un seul bouton « Delete » existe
		// (la ConfirmModal n'est pas encore ouverte).
		await user.click(screen.getByRole("button", { name: /^delete$/i }));

		// ConfirmModal porte 3 (titre « Delete date »). Le popover reste
		// ouvert (la porte 3 avec-données n'appelle pas closePopover), mais
		// `findByRole('dialog')` cible le Modal qui porte role="dialog".
		const dialog = await screen.findByRole("dialog");
		expect(dialog).toHaveTextContent(/delete date/i);
	});
	...
	it("édition : supprimer une date manuelle sans données ne confirme pas", async () => {
		...
		expect(
			screen.queryByRole("button", {
				name: new RegExp(formatBadgeDate("2026-08-22").replace(/\./g, "\\."), "i")
			})
...
describe("PlanningForm — Porte 4 (requestDisableSlot)", () => {
	it("édition : désactiver une DateSlot avec données ouvre la ConfirmModal", async () => {
		...
		// « Disable » (la DateSlot est sélectionnée).
		await user.click(screen.getByRole("button", { name: /^disable$/i }));

		// ConfirmModal porte 4 (titre « Remove this date »).
		const dialog = await screen.findByRole("dialog");
		expect(dialog).toHaveTextContent(/remove this date/i);
	});
	...
	it("édition : désactiver une DateSlot sans données ne confirme pas", async () => {
		...
		// « Reactivate » (la DateSlot est désactivée → `isSelected = false`).
		await user.click(getDateBadge("2026-08-12"));
...
describe("PlanningForm — Porte 6 (applySlotEdit)", () => {
	it("édition : appliquer de nouveaux horaires ouvre la ConfirmModal de propagation", async () => {
		...
		await user.click(screen.getByRole("button", { name: /edit slot times/i }));

		// Change les horaires via le preset « Morning » (08:00 – 12:00), plus
		...
		// « Apply » dans le modal slot → en mode edit + master :
		// closeSlotModal() puis openConfirm() (porte 6).
		await user.click(screen.getByRole("button", { name: /^apply$/i }));

		// ConfirmModal porte 6 (titre « Apply new times »).
		const dialog = await screen.findByRole("dialog");
		expect(dialog).toHaveTextContent(/apply new times/i);
	});
