...
{:else if master}
  ...
  <NetworkAlert />
    ...
    <div class="mb-4 sm:mb-12">
      <div
        class="mb-2 flex flex-wrap items-start justify-between gap-6 sm:mb-8"
      >
        <div class="flex flex-1 items-center gap-5">
          ...
          <div class="flex-1 space-y-1">
            <h1 class="text-xl font-semibold tracking-tight sm:text-4xl">
              {master.title}
            </h1>
            <div
              class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium opacity-80"
            >
              {#if master.place}
            ...
            <div class="flex min-w-[calc(50%-0.5rem)] items-center gap-2">
              <div class="flex flex-wrap items-center gap-x-2">
                <div class="truncate text-sm font-medium">
                  {getRecurrenceLabel(master.recurrence)} • {master.defaultStartTime}
                  — {master.defaultEndTime}
                </div>
                <div class="text-base-content/70 text-sm">
                  {#if master.recurrence.lastDate}
                    {m.participant_until()}
                    {formatDateShort(master.recurrence.lastDate)}
                  {/if}
                </div>
              </div>
            </div>
      ...
      <div
        class="card card-sm bg-base-300/20 border-base-content/5 mb-4 border shadow-sm"
      >
        <div class="card-body gap-4">
          <!-- Groupe « Vous » : gestion de l'identité courante -->
          <div>
            <div
              class="text-content-primary mb-2 flex items-center gap-2 text-sm font-semibold"
            >
            ...
            {:else if identityClaimedByAuth}
              ...
              <div
                class="alert alert-warning alert-soft flex items-center gap-3 py-2"
              >
            ...
            {:else}
              <!-- Non identifié -->
              <div
                class="alert alert-outline alert-warning flex flex-wrap items-center gap-3"
              >
                <span class="text-sm opacity-80"
                  >{m.participant_not_identified()}</span
                >
                <button
    ...
    <div class="">
        ...
        <div class="mx-2 flex gap-2">
          ...
          {#if mediaQuery.isMobile}
            <button
              ...
              onclick={() =>
                shareLink(
                  `${window.location.origin}/p/${master!.participantToken}`,
                  m.participant_share_public_link(),
                )}
            >
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold max-sm:px-2 sm:text-2xl">
          {m.participant_upcoming_dates()}
        </h2>
        <ViewTabs />
      </div>
