...
const { minPresentRequired: masterMinPresentRequired, tasks: masterTasks = [] } = (() => master)();
...
async function confirmResponseChange() {
	...
	await applyResponseChange(pending.participantId, pending.targetResponse, pending.onEventTaskIds);
}
...
async function handleSubmit() {
	...
	try {
		const updates: Partial<PlanningOccurrence> = {
			...
			isConfirmed,
			isCanceled,
			minPresentRequired: minPresentRequired !== master.minPresentRequired ? minPresentRequired : 0,
        ...
        <div class="space-y-2 md:max-w-1/2">
          <label class="label-text font-medium"
            ...
            <div class="flex items-center gap-4">
              ...
              />
              <span class="badge badge-primary tabular-nums"
                >{minPresentRequired}</span
              >
            </div>
          </label>
        </div>
            ...
            {#each master.participants as participant (participant.id)}
              {#key participant.id}
                ...
                <div class="bg-base-200 rounded-box px-4 py-1">
                  <div
                    class="flex items-center justify-between gap-4 max-sm:flex-col"
                  >
