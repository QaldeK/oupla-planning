...
const TASK_TYPE_CONFIG: Record<TaskType, { bgClass: string; label: string; icon: typeof Clock }> = {
	beforeEvent: {
		...
		label: m.task_type_before(),
		icon: CalendarArrowUp
	},
	onEvent: {
		bgClass: "bg-accent/60",
		label: m.task_type_during(),
		icon: Clock
	},
	afterEvent: {
		bgClass: "bg-accent",
		label: m.task_type_after(),
...
{#if displayMode === "card"}
  <div class="mb-3 flex flex-wrap items-center gap-x-6 gap-y-2">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <div class="badge bg-accent/30">
        <CalendarArrowUp class="size-4" />
        {m.task_type_before()}
      </div>
      <div class="badge bg-accent/60">
        <Clock class="size-4" />
        {m.task_type_during()}
      </div>
    ...
    >
      ...
      <div class="ms-auto flex items-center gap-4">
        <div
          ...
          aria-label={m.task_required_count_aria()}
    ...
    <div class="flex flex-1 flex-wrap items-center gap-1.5 p-3">
      {#if volunteers > 0}
        {#each inscribed as response (response.participantId)}
          ...
          <div
            class="badge md:badge-lg bg-accent/60 {response.participantId ===
            currentUserId
  ...
  >
    ...
    >
      ...
      <div class="flex flex-col items-start justify-start">
        ...
        <div class="truncate text-start leading-none text-wrap">
          {task.name}
        </div>
      </div>
    </div>
    {#if volunteers > 0}
      ...
      {#each displayInscribed as response (response.participantId)}
        {@const isQuit = quitParticipantIds.has(response.participantId)}
        <div
          class="badge bg-accent/60 m-1.5 {response.participantId ===
          currentUserId
            ? 'border-accent border-2 font-bold'
            : 'font-medium'} {isQuit ? 'line-through opacity-40' : ''}"
          transition:slide
      ...
      {#if volunteers > 6}
        ...
        {@const hiddenNames = hiddenParticipants
          .map((r) => getParticipantName(r))
          .join(", ")}
    ...
    <div class="ms-auto flex items-center gap-2 p-1.5">
      <div
        class="badge badge-sm font-semibold {isComplete
          ? 'badge-success'
          : 'badge-warning'} px-1"
        title={m.task_required_count_title({ name: task.name })}
      >
...
{#if tasks && tasks.length > 0}
  {#if isMinimalDisplay}
    <fieldset
      {disabled}
      class="my-1 flex w-full flex-wrap gap-2 {disabled &&
  ...
  {:else}
    <fieldset
      {disabled}
      class="flex w-full flex-wrap gap-3 {disabled &&
        'opacity-70 grayscale-50'}"
    >
      {#if isCompactDisplay}
        <legend class="mb-1 text-xs opacity-60">{m.task_list_title()}: </legend>
