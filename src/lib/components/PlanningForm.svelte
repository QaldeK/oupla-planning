			...
			<div class="space-y-3">
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
					{#each timeSlots as slot (slot.id)}
						<div class="bg-base-200 flex items-center justify-between gap-2 rounded-lg px-3 py-2">
							<div class="flex items-center gap-2">
	...
	>
		...
		<button type="submit" class="btn btn-primary px-8" disabled={isSubmitting}>
			...
			<span class="md:hidden">
				{master ? msg.planform_submit_edit_mobile() : msg.planform_submit_create_mobile()}</span
			>
		</button>
	</div>
</form>
